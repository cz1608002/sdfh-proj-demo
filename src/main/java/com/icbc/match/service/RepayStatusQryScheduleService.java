...
@Slf4j
public class RepayStatusQryScheduleService {
    ...
    private void configureRpStTasks() {


        String transcode = "LOANLIST";
  //      String merid = ApiConstants.API_YKGYJ_MERID;
        ...
        String queryendtime = "";
        String apiname = ApiConstants.API_YKGYJ_COM_ICBC_GYJ_ELOANSPLUS_LOANLIST;
        ...
        ArrayList<CreditQryList> AccountCoderlist = null;

        try {
            AccountCoderlist = creditQryMapper.selectAccountCoderList();

            if (AccountCoderlist.size() > 0) {
                for (int i = 0; i < AccountCoderlist.size(); i++) {
                    ...
                    String merid = AccountCoderlist.get(i).getMerid();
                    String orderno = AccountCoderlist.get(i).getAccountCode();
                    String reqdata = "<GYJ>" +
                            "<pub>" +
                                    ...
                                    if ("200".equals(retCode)) {
                                        try {
                                            ...
                                            if ("0".equals(totalCount)) {
                                                BillStatus = "0";
                                                log.info("订单accountCode" + orderno + "无还款信息" + queryendtime);
                                                continue;
                                            ...
                                            for (Element rdnode : rdnodes) {
                                                ...
                                                String ExpiredDate = rdnode.elementText("ExpiredDate");
                                                ed = df.format(df8.parse(ExpiredDate));

                                                ed15 = df.format(df8.parse(ExpiredDate).getTime()-15*24*60*60*1000);


                                                if ("0".equals(BillStatus)
                                                        ||"1".equals(BillStatus)||"2".equals(BillStatus)
                                                        ||"3".equals(BillStatus)||"5".equals(BillStatus)
                                                        ||"6".equals(BillStatus)||"7".equals(BillStatus)
                                                        ||"8".equals(BillStatus)||"9".equals(BillStatus)){

                                                    BillStatus = "1";//未还款
                                                }else {
//                                                    if (trandate.compareTo(ed)<0){
//                                                        BillStatus = "2";//提前还款
//                                                    }else if(trandate.compareTo(ed)==0){
//                                                        BillStatus = "3";//到期还款
//                                                    }
                                                    BillStatus = "3";
                                                }

                                                System.out.println("申请编号为" + orderno + "的订单还款状态为" + BillStatus);

                                                //更新account_detail表还款状态
                                                try {
                                                    int num = 0;

//                                                        int num = accountDetailMapper.updateRepayStatus(BillStatus, orderno);
//                                                        if (num > 0) {
//                                                            log.info("订单" + orderno + "还款状态更新成功");

                                                            if ("1".equals(BillStatus) && trandate.compareTo(ed15) !=0){
                                                                log.info("订单" + orderno + "未还款且未到还款日");
                                                                continue;
                                                            }
                                                            //发送第三方
                                                            String yknoticetUrl = ApiConstants.API_YK_HXQY_UPDATEFINISH;
                                                      ...
                                                      //      String htcode = CreditQryList.get(0).getContractNo();
                                                            ...
                                                            if ("10000".equals(code)) {
                                                                log.info("兖矿接收还款信息成功");
                                                                msg = "兖矿接收还款信息成功";
                                                                num = accountDetailMapper.updateLoanBak2(msg,BillStatus,orderno);
                                                                log.info("订单" + orderno + "还款状态更新成功");
                                                                continue;
                                                                ...
                                                                //更新状态为未还款
                                                                BillStatus = "1";
                                                                num = accountDetailMapper.updateRepayStatus1(msg, BillStatus, orderno);
                                                                continue;
                                                            }


//                                                        } else {
//                                                            log.info("订单" + orderno + "还款状态更新失败");
//                                                            continue;
//                                                        }
                                ...
                                log.info("没有新增已还款订单" + queryendtime);
                                continue;
                ...
                log.info("没有需要还款的订单" + queryendtime);

            }
        } catch (Exception e) {
            e.printStackTrace();
            log.info("查询未还款订单出错" + e.getMessage());
        }
    }
