...
@Service
@Slf4j
public class SettlementAccountScodeV1VerifyService {
    ...
    public SettlementAccountSCodeVerifyResponseV1 execute(SettlementAccountSCodeVerifyRequestV1.SettlementAccountSCodeVerifyRequestV1Biz bizContent) {

        SettlementAccountSCodeVerifyRequestV1 request = new SettlementAccountSCodeVerifyRequestV1();

        String serviceUrl = apiServer + ApiConstants.API_SETTLEMENT_ACCOUNT_SCODE_V1_VERIFY;
        request.setServiceUrl(serviceUrl);
        request.setBizContent(bizContent);
        try {
            SettlementAccountSCodeVerifyResponseV1 response = apiClientCopService.execute(request);
