...
@Service
@Slf4j
public class SettlementAccountDetailV1QueryService {
    ...
    public SettlementAccountDetailQueryResponseV1 execute(SettlementAccountDetailQueryRequestV1.SettlementAccountDetailQueryRequestV1Biz bizContent) {

        SettlementAccountDetailQueryRequestV1 request = new SettlementAccountDetailQueryRequestV1();

        String serviceUrl = apiServer + ApiConstants.API_SETTLEMENT_ACCOUNT_DETAIL_V1_QUERY;
        request.setServiceUrl(serviceUrl);
        request.setBizContent(bizContent);
        try {
            SettlementAccountDetailQueryResponseV1 response = apiClientCopService.execute(request);
