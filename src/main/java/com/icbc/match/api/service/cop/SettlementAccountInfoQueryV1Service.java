...
@Service
@Slf4j
public class SettlementAccountInfoQueryV1Service {
    ...
    public SettlementAccountInfoQueryResponseV1 execute(SettlementAccountInfoQueryRequestV1.SettlementAccountInfoQueryRequestV1Biz bizContent) {

        SettlementAccountInfoQueryRequestV1 request = new SettlementAccountInfoQueryRequestV1();

        String serviceUrl = apiServer + ApiConstants.API_SETTLEMENT_ACCOUNT_INFO_QUERY_V1;
        request.setServiceUrl(serviceUrl);
        request.setBizContent(bizContent);
        try {
            SettlementAccountInfoQueryResponseV1 response = apiClientCopService.execute(request);
